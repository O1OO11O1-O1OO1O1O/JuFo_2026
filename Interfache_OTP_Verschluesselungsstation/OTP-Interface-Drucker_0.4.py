...
usb_pfad = Path("/media/marten/INTENSO/zahlen.txt")
...
def Einstellungen():
    ...
    def SchlüsselUpdate():
        ...
        if usb_pfad.exists():
            print("usb_pfad-Stick ist angeschlossen, Schlüsselspeicher wird Aktualisiert")
            ...
            print("usb_pfad-Stick nicht vorhanden")
    ...
    textfeld_SD.insert(tk.END, usb_pfad)
    ...
    def Speichern():
        usb_pfad = Path(textfeld_SD.get("1.0", "end-1c"))
...
def Drucken():
    ...
    print("vs:", "".join(verschluesselterSatz))

    if verschluesselterSatz != []:

        drucker.set(align='center')
        drucker.text("\n\n\nDeine Verschlüsselte Nachricht\n")

        drucker.qr("".join(verschluesselterSatz), size=6)

        drucker.text("\n\nText:\n")
        drucker.text("".join(verschluesselterSatz))
        drucker.text("\n\n\n\n\n ")

    else:
        messagebox.showerror('Kein Text verschlüsselt', 'Verschlüssele erst einen Text zum ausdrucken\n' )
...
button_Drucker.place(x=1600, y=600, width=110, height=300)
...
textfeld.place(x=210, y=600, width=1360, height=300)
